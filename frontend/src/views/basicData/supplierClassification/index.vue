...
<script>
  ...
  import { supplierClassificationExport } from '@/api/export'
  import { supplierClassificationTemplate, supplierClassificationImport } from '@/api/import'
  import { supplierClassificationList, supplierClassificationDestroy } from '@/api/basicData'
  import { getSupplierDictNumber } from '@/api/data'
  ...
  export default {
    ...
    methods: {
      ...
      list() {
        this.loading = true;
        supplierClassificationList(this.searchForm).then(data => {
      ...
      openFormModal(item) {
        // if (!item.id) {
        //   getSupplierDictNumber().then(data => {
      ...
      destroy(id) {
        supplierClassificationDestroy({ id }).then(() => {
          this.items.splice(this.items.findIndex(item => item.id == id), 1);
          this.$message.success('删除成功');
        });
      },
      exportExcel() {
        supplierClassificationExport(this.searchForm).then(resp => {
          exportExcel(resp.data, '供应商分类列表');
        }).catch(err => {
          this.$message.error(err.response.data.error);
        });
      },
      downloadTemplate () {
        supplierClassificationTemplate().then(resp => {
          exportExcel(resp.data, '供应商分类导入模板');
        }).catch(err => {
          this.$message.error(err.response.data.error);
        });
      },
      importExcel(item) {
        ...
        setTimeout(() => {
          supplierClassificationImport(data)
            .then(() => {
