...
<script>
  ...
  import { clientClassificationExport } from '@/api/export'
  import { clientClassificationTemplate, clientClassificationImport } from '@/api/import'
  import { clientClassificationList, clientClassificationDestroy } from '@/api/basicData'
  ...
  export default {
    ...
    methods: {
      ...
      list() {
        this.loading = true;
        clientClassificationList(this.searchForm).then(data => {
      ...
      destroy(id) {
        clientClassificationDestroy({ id }).then(() => {
          this.items.splice(this.items.findIndex(item => item.id == id), 1);
          this.$message.success('删除成功');
        });
      },
      exportExcel() {
        clientClassificationExport(this.searchForm).then(resp => {
          exportExcel(resp.data, '客户分类列表');
        }).catch(err => {
          this.$message.error(err.response.data.error);
        });
      },
      downloadTemplate () {
        clientClassificationTemplate().then(resp => {
          exportExcel(resp.data, '客户分类导入模板');
        }).catch(err => {
          this.$message.error(err.response.data.error);
        });
      },
      importExcel(item) {
        ...
        setTimeout(() => {
          clientClassificationImport(data)
            .then(() => {
