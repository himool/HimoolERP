  ...
  <div>
    <a-card :tab-list="tabList" :active-tab-key="currentTab" @tabChange="changeTab">
      ...
      <a-descriptions :column="{ xxl: 4, xl: 4, lg: 4, md: 2, sm: 2, xs: 1 }" style="text-align: center;">
        <a-descriptions-item label="销售总量"><span class="total">{{totalQuantity}}</span></a-descriptions-item>
        <a-descriptions-item label="销售额"><span class="total">{{totalAmount}}</span></a-descriptions-item>
        <a-descriptions-item label="销售毛利"><span class="total">{{NP.round(totalAmount - totalCost, 2)}}</span></a-descriptions-item>
        <a-descriptions-item label="销售毛利率"> <span class="total">{{grossRate}} </span>%</a-descriptions-item>
      </a-descriptions>
...
<script>
  import { salesReportList } from '@/api/report'
  ...
  export default {
    ...
    computed: {
      ...
      grossRate() {
        return this.total.amount ? NP.round(NP.divide(NP.minus(this.totalAmount, this.totalCost), this.totalAmount, 0.01), 2) : 0
      },
    },
    methods: {
      initailize() {
