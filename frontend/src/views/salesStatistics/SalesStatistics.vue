<template>
  <div>
    <a-card>
      <a-row gutter="16">
        <a-col :span="8">
          <a-select v-model="searchForm.client" show-search placeholder="客户" :disabled="tableLoading || cardLoading"
            style="width: 100%;" @change="search" @search="searchClient" allowClear>
    ...
    <a-row gutter="8" style="margin-top: 8px;">
      <a-col :span="6">
        <a-card hoverable>
          <a-statistic title="应收金额(元)" :value="statistics.total_amount" :loading="cardLoading" />
        </a-card>
      </a-col>
      <a-col :span="6">
        <a-card hoverable>
          <a-statistic title="已收金额(元)" :value="statistics.amount" :loading="cardLoading" />
        </a-card>
      </a-col>
      <a-col :span="6">
        <a-card hoverable>
          <a-statistic title="待收金额(元)" :value="NP.minus(statistics.total_amount, statistics.amount)"
            :loading="cardLoading" />
        </a-card>
      </a-col>
      <a-col :span="6">
        <a-card hoverable>
          <a-statistic title="退款金额(元)" :value="statistics.return_amount" :loading="cardLoading" />
        </a-card>
      </a-col>
    </a-row>
    <a-row style="margin-top: 8px;">
      <a-card title="销售收款统计">
        <a-table :columns="columns" :data-source="items" :loading="tableLoading" :pagination="false" size="small"
          ...
          <div slot="is_return" slot-scope="value">{{value ? '退货单' : '销售单'}}</div>
          <div slot="remain_amount" slot-scope="value, item">{{NP.minus(item.total_amount, item.amount)}}</div>
          <div slot="action" slot-scope="value, item">
            <a-button size="small" type="primary" @click="payment(item)">收款</a-button>
          </div>
        </a-table>
...
<script>
  import { clientList, salesOrderList, paymentRecordCreate } from '@/api/sales'
  import { salesStatistics } from '@/api/report'
  ...
  export default {
    ...
    data() {
      return {
        NP,
        moment,
        searchForm: { page: 1, ordering: '-date', search: '', client: undefined },
        clientItems: [],
        items: [],
        ranges: {
    ...
    methods: {
      ...
      list() {
        ...
        salesStatistics(this.searchForm)
          .then(resp => {
      ...
      changeDateRange(dateRange) {
        this.searchForm.start_date = moment(dateRange[0]).format('YYYY-MM-DD');
        this.searchForm.end_date = moment(dateRange[1]).format('YYYY-MM-DD');
        this.search();
      },
    },
    mounted() {
