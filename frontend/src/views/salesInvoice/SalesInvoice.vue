    ...
    <div v-else style="text-align: center">
      <div id="invoice" style="padding: 24px">
        ...
        <a-row style="font-size: 15px; margin-top: 8px">
          ...
          <a-col :span="6"
            >日期: {{ moment(item.date).format("YYYY-MM-DD") }}</a-col
          >
        </a-row>
        <a-row style="margin-top: 8px">
          <a-table
            :columns="columns"
            :data-source="item.goods_set"
            size="small"
            :pagination="false"
            :loading="loading"
            bordered
        ...
        <a-row style="margin: 16px">
          ...
          <a-col :span="12">
            <div style="font-size: 15px; float: right">
              总合计: ¥ {{ NP.times(totalAmount, item.discount, 0.01) }}
            </div>
          </a-col>
        </a-row>
      </div>
    ...
    totalAmount() {
      ...
      if (this.item.goods_set && this.item.goods_set.length > 0) {
        ...
        for (let goods of this.item.goods_set) {
          totalAmount = NP.plus(
            totalAmount,
            NP.times(goods.quantity, goods.retail_price)
          );
          rowIndex++;
          goods.index = rowIndex; 
        }
      }
