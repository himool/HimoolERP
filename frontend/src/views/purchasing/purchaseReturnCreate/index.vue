...
<script>
  ...
  import { suppliersOption, userOption, warehousesOption, inventoriesOption, accountsOption, purchaseOrdersOption } from '@/api/option'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          number: [
            { required: true, message: '请输入退货单编号', trigger: 'change' },
          ],
          warehouse: [
      ...
      create() {
        this.$refs.form.validate(async (valid) => {
          if (valid) {
            ...
            purchaseReturnOrderCreate(formData).then(data => {
