<template>
  <div>
    <a-row gutter="8">
      <a-col :span="8">
        <a-card>
          <a-row gutter="16">
            ...
            <a-col :span="12" style="margin-bottom: 8px;">
              <a-select v-model="searchForm.is_return" placeholder="单据类型" style="width: 100%;" allowClear>
                <a-select-option :value="false">销售单</a-select-option>
                <a-select-option :value="true">销售退货单</a-select-option>
              </a-select>
            </a-col>
            <a-col :span="24" style="margin-bottom: 16px;">
              <a-input-search v-model="searchForm.search" allowClear enter-button @search="search"
                placeholder="客户姓名 / 联系电话 / 销售员/ 订单备注" />
            </a-col>
          </a-row>
          <a-row>
            <a-table :columns="salesColumns" :data-source="items" :loading="loading" :pagination="false"
              :customRow="customRow" :rowClassName="rowClassName" size="small">
              <div slot="status" slot-scope="value, item">
                {{item.is_return ? '退货单' : '销售单'}} - {{item.is_done ? '已完成' : item.is_return ? '等待入库' : '等待出库'}}
              </div>
      ...
      <a-col :span="16">
        <a-card :title="form.id ? form.is_return ? `销售退货单 - ${form.id}` : `销售单 - ${form.id}` : '销售单'">
          <div v-if="form.is_return">
            <div v-if="formLoading" style="text-align: center; margin-top: 36px;">
              <a-spin />
            </div>
            <div v-else>
              ...
              <a-row style="margin-top: 12px;">
                <a-table :columns="goodsColumns" :data-source="salesForm.goods_set" :pagination="false" size="small">
                  <div slot="index" slot-scope="value, item, index">{{index + 1}}</div>
                  <div slot="retail_price" slot-scope="value">{{NP.round(value, 2)}}</div>
          ...
          <a-row style="margin-top: 12px;">
            <a-table :columns="goodsColumns" :data-source="dataSource" :pagination="false" size="small">
              ...
              <div slot="retail_price" slot-scope="value, item">{{item.isTotal ? '' : NP.round(value, 2)}}</div>
              <div slot="amount" slot-scope="value, item">
                {{item.isTotal ? value : NP.round(NP.times(item.quantity, item.retail_price), 2)}}
              </div>
            </a-table>
          </a-row>
          <a-row style="margin-top: 12px;">
            <a-popconfirm title="确定删除吗?" @confirm="destroy">
              <a-button v-if="form.id && !form.is_done" type="danger">删除</a-button>
            </a-popconfirm>
            <a-button v-if="form.id && form.is_done" style="margin-left: 12px;" @click="printInvoice">生成打印单据</a-button>
          </a-row>
        </a-card>
      </a-col>
    </a-row>
...
<script>
  ...
  import { salesOrderList, salesOrderRetrieve, salesOrderDestroy } from '@/api/sales'
  ...
  export default {
    ...
    methods: {
      ...
      retrieve() {
        ...
        salesOrderRetrieve({ id: this.form.sales_order })
          .then(resp => {
