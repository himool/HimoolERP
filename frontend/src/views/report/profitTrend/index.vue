<template>
  <div>
    <a-card style="height: calc(100vh - 120px)">
      <div slot="title">
        <span>利润走势</span>
...
<script>
  import { profitTrends } from '@/api/report'
  ...
  export default {
    ...
    methods: {
      initialize() {
        ...
        this.chart.scale({ date: { range: [0, 1] }, total_profit_amount: { nice: true } });
        this.chart.tooltip({ showCrosshairs: true, shared: true });
        this.chart.line().position('date*total_profit_amount').color('warehouse_name').shape('smooth');
        this.chart.point().position('date*total_profit_amount').color('warehouse_name').shape('circle');
        ...
        for (let i = 0; i <= days; i++) {
          for (let w of warehouseList) {
            if (items.findIndex(item => item.warehouse_name === w && item.date === startDate.format('YYYY-MM-DD')) === -1) {
              fillItems.push({ date: startDate.format('YYYY-MM-DD'), warehouse_name: w, total_profit_amount: 0 });
            }
          }
