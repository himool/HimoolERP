    ...
    <a-card>
      ...
      <div>
        <a-table :columns="columns" :data-source="items" size="small" :pagination="false" :loading="loading"
          ...
          <div slot="start_date" slot-scope="value">{{moment(value).format(dateTimeFormat)}}</div>
          <div slot="end_date" slot-scope="value">{{moment(value).subtract(1, 'days').format(dateTimeFormat)}}</div>
          <div slot="create_date" slot-scope="value">{{moment(value).format(dateTimeFormat)}}</div>
        </a-table>
      </div>
