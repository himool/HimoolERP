<template>
  <div>
    <a-modal v-model="visible" title="初始化库存" :maskClosable="false" @cancel="cancel" @ok="confirm">
      <div style="margin-top: 12px;">
        <a-table :columns="columns" :data-source="items" :loading="loading" size="small" :pagination="false">
          <div slot="quantity" slot-scope="value, item">
            <a-input-number :value="item.quantity" min="0" @change="value => changeQuantity(value, item)" />
          </div>
        </a-table>
      </div>
    </a-modal>
        ...
        warehouseList({ status: true })
          .then(resp => {
            ...
            for (let item of this.items) {
              item.quantity = this.inventory[item.id] ? this.inventory[item.id] : 0;
            }
          })
          .catch(err => {
